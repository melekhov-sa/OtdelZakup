...
qty/uom policy (strict, no defaults):
  1. Try qty column (combined "1500 шт" or plain numeric).
...
def parse_row(cells: dict[str, Any], mapping: dict[str, Any]) -> dict:
    ...
    from app.parser_excel import extract_qty_uom_suffix, parse_qty_uom  # noqa: PLC0415
    ...
    known_cols = {name_col, qty_col, code_col, standard_col, strength_col, note_col} - {None}
    ...
    if qty_cell_str:
        if qty_is_combined:
            ...
            try:
                qty = float(clean)
                # uom still unknown; will try name / raw_text next
            except ValueError:
