...
_QTY_SYNONYMS = [
    ...
    "необходимо заказать", "необходимо",
]
...
def parse_excel(file_path: str | Path) -> ParseResult:
    ...
    if standard_idx is not None:
        # Validate by content: at least one cell must contain a standard keyword.
        # Prevents date/numeric columns from being mistakenly mapped as standard.
        _std_kw = re.compile(r"din|iso|гост|gost", re.IGNORECASE)
        _std_vals = [
            str(values_2d[r][standard_idx])
            for r in range(data_start, len(values_2d))
            if standard_idx < len(values_2d[r]) and values_2d[r][standard_idx] is not None
        ]
        if not any(_std_kw.search(v) for v in _std_vals):
            standard_idx = None
    if standard_idx is not None:
