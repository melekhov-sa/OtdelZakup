...
## 9. Справочники, правила и настройки

Они хранятся в `readiness.db` и **с кодом не едут**: `git pull` их не приносит,
а новая установка стартует с пустыми справочниками. Так, например, режим «Поиск
по DIN» на новом сервере не работает, пока не заведены пары аналогов стандартов.

Перенос между серверами и возврат после переустановки — через страницу
`/settings/backup`:

- **Выгрузить настройки** на сервере-источнике — скачается файл со всеми
  справочниками, правилами и настройками
- **Загрузить** на сервере-приёмнике — содержимое файла **полностью заменит**
  справочники. Перед заливкой автоматически сохраняется копия текущего состояния
  в `data/backups/`

Каталог номенклатуры в снимок не входит — он восстанавливается синхронизацией
из 1С. Память подбора тоже не переносится: она привязана к позициям конкретного
сервера.

Для снимков по расписанию есть консольный вариант:

```powershell
.\venv\Scripts\python.exe scripts\export_settings.py "D:\OtdelZakup\backups\settings.json"
```
